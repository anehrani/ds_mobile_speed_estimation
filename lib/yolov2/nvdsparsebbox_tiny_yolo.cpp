        ...
        std::vector<NvDsInferParseObjectInfo>& objectList)
{
    ...
    // check the number of detections befor filling

    //objectList = nmsAllClasses(objects, kNUM_CLASSES_YOLO, kNMS_THRESH);
    nmsAllClasses(objects, kNUM_CLASSES_YOLO, kNMS_THRESH);



    objectList.push_back( { 1, 90.2, 130.8, 65.9, 35.5, 0.9} );
    
    std::cout<< " --- inside yolov2, Num detections: " << objectList.size() << "  ----" << std::endl;
